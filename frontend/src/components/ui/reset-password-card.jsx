...
import { Lock, Eye, EyeClosed, ArrowRight, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export function ResetPasswordCard({ onSubmit, isLoading: externalLoading, error: externalError, success: externalSuccess, invalidToken }) {
                        ...
                        <div className="relative bg-black/40 backdrop-blur-xl rounded-2xl p-6 border border-white/[0.05] shadow-2xl overflow-hidden">
                            ...
                            {externalSuccess && (
                                <motion.div
                                    initial={{ opacity: 0, height: 0 }}
                                    animate={{ opacity: 1, height: 'auto' }}
                                    className="mb-4 p-3 rounded-lg bg-green-500/10 border border-green-500/20 flex items-start gap-2 text-green-200 text-sm"
                                >
                                    <CheckCircle2 className="w-4 h-4 text-green-400 shrink-0 mt-0.5" />
                                    <div>
                                        <p className="font-semibold">Success!</p>
                                        <p className="text-xs mt-1">{externalSuccess}</p>
                                    </div>
                                </motion.div>
                            )}
                            ...
                            {invalidToken ? (
                                <Link
                                    to="/forgot-password"
                                    className="w-full relative group/button mt-5 block"
                                >
                                ...
                                <form onSubmit={handleSubmit} className="space-y-3">
                                    ...
                                    {/* Confirm Password */}
                                    <motion.div
                                        className={`relative ${focusedInput === "password_confirm" ? 'z-10' : ''}`}
                                        whileFocus={{ scale: 1.02 }}
                                        whileHover={{ scale: 1.01 }}
                                        transition={{ type: "spring", stiffness: 400, damping: 25 }}
                                    >
                                        <div className="relative flex items-center overflow-hidden rounded-lg">
                                            <Lock className={`absolute left-3 w-4 h-4 transition-all duration-300 ${focusedInput === "password_confirm" ? 'text-white' : 'text-white/40'
                                                }`} />
