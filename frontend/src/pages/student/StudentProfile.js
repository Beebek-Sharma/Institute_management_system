    ...
    <DashboardLayout>
      <div className="min-h-screen bg-transparent">
        ...
        <div className="bg-white backdrop-blur-md border-b border-gray-200 p-6 mb-8 rounded-lg">
          ...
          <p className="text-gray-900">View your profile information</p>
        </div>

        {/* Profile Card */}
        <div className="bg-white backdrop-blur-md border border-gray-200 rounded-lg p-8 max-w-2xl">
          {/* Profile Header with Avatar */}
          <div className="flex flex-col items-center mb-8 pb-8 border-b border-gray-200">
            <div className="w-24 h-24 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-gray-900 text-3xl font-bold mb-4">
              {user?.first_name?.[0]}{user?.last_name?.[0]}
            </div>
            <h2 className="text-3xl font-bold text-gray-900 mb-2">{user?.first_name} {user?.last_name}</h2>
            <p className="text-gray-700 text-lg">{user?.role ? user.role.charAt(0).toUpperCase() + user.role.slice(1) : 'User'}</p>
          </div>
            ...
            <div className="flex items-start gap-4 pt-4">
              ...
              <div className="flex-1">
                ...
                <span className={`inline-block px-4 py-2 rounded-full text-sm font-semibold ${
                  user?.role === 'admin' ? 'bg-red-500/30 text-red-800' :
